    ...
    public class I18n : INotifyPropertyChanged
    {
        ...
        private static readonly Dictionary<string, Dictionary<string, string>> Translations = new()
        {
            { "zh", new Dictionary<string, string> {
                ...
                { "AboutChangelog", "• 新增：设置页支持配置关闭到托盘和修改全局快捷键\n• 新增：布局列表支持置顶和排序功能（置顶、上移、下移）\n• 优化：改进桌面图标匹配算法，采用两阶段定位策略提升恢复准确性\n• 修复：系统关机/重启时应用未正常退出的问题\n• 修复：托盘恢复桌面失败时的潜在崩溃风险" },
            ...
            { "en", new Dictionary<string, string> {
                ...
                { "AboutChangelog", "\u2022 New: Settings page supports \"Close to Tray\" toggle and hotkey customization\n\u2022 New: Layout pin-to-top and reorder (pin, move up, move down)\n\u2022 Improved: Enhanced icon matching with two-stage positioning for better restore accuracy\n\u2022 Fixed: Application not exiting properly on system shutdown/restart\n\u2022 Fixed: Potential crash when tray restore operation fails" },
