    ...
    public class I18n : INotifyPropertyChanged
    {
        ...
        private static readonly Dictionary<string, Dictionary<string, string>> Translations = new()
        {
            { "zh", new Dictionary<string, string> {
                ...
                { "AboutChangelog", "• 新增：全局快捷键一键恢复快照功能（默认 Ctrl+Alt+R，可在设置中自定义）\n• 新增：快照列表右键支持设置固定恢复目标\n• 优化：引入快照内存缓存机制，大幅提升列表加载与切换流畅度\n• 修复：多显示器环境下（副屏位于左侧或上方）桌面预览图标错位的问题\n• 修复：规范化版本号检测与更新比对逻辑" },
            ...
            { "en", new Dictionary<string, string> {
                ...
                { "AboutChangelog", "\u2022 New: Global hotkey for instant snapshot restoration (default: Ctrl+Alt+R, customizable in Settings)\n\u2022 New: Support setting a designated restore target snapshot via context menu\n\u2022 Improved: In-memory layout caching for significantly faster loading and smoother UI\n\u2022 Fixed: Desktop preview icon displacement issue in multi-monitor setups with negative coordinates\n\u2022 Fixed: Normalized version comparison for update detection" },
